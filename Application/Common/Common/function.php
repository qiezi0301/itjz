...
function getSign($row) {
    ...
    if ($t > date('d')) {
        ...
        $info = M("sign")->field("id")->where("addtime = " . $t2 . " AND status = 0 AND uid = " . $uid . "")->find();

        if ($info) {
            $td = "<td style='background-color:navajowhite;navajowhite ;'>
            <div align='right' valign='top'><span style='position:relative;right:20px;'>" . $t . "</span>
            </div><div align='left'>
            <img width='35px' height='35px' src='" . C('TMPL_PARSE_STRING.__IMG__') . "/cart_3.gif' style='position:relative;left:10px;'> 已签到
            </div></td>";
        } else {
            if ($t == date('d')) {
