...
class Category {
	...
	//获取顶级分类
	static public function getParentsTop($cate) {
		$arr = array();
		foreach ($cate as $v) {
			if ($v['pid'] == 0) {
				$arr[] = $v;				
	...
	**/
	...
	//传递一个父级分类ID返回所有子孙级分类首页推荐分类
	static public function getChildsIsindex($cate, $pid) {
		$arr = array();
		foreach ($cate as $v) {
			if ($v['pid'] == $pid) {
				if ($v['isindex'] == 1) {
					...
					$arr = array_merge($arr, self::getChildsIsindex($cate, $v['id']));
			}
		}
		return $arr;
	}
