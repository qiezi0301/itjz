...
class YunUpload
{
    ...
    private $allowType    = array('jpg', 'gif', 'png', 'jpeg', 'zip', 'rar'); //允许类型
    ...
    public function __construct($thum_flag = 0, $sub_path = 'file1', $file_field = '')
    {
        ...
        //上传根目录
        $this->rootPath = C('CFG_UPLOAD_ROOTPATH') ? C('CFG_UPLOAD_ROOTPATH') : './Uploads/';
        //带缩图的图片，生成为固定位置
        if ($this->thumFlag) {
        ...
        if (empty($_FILES)) {
            ...
            if (isset($info) && is_array($info)) {
                ...
                foreach ($info as $k => $v) {
                    ...
                    if ($this->thumFlag) {
                        //读取缩略图配置信息
                        $imgtbSize = explode(',', (C('CFG_IMGTHUMB_SIZE') ? C('CFG_IMGTHUMB_SIZE') : '300X300')); //配置缩略图第一个参数
    ...
    public function _upload()
    {
        ...
        // $upload->exts     = array('jpg', 'gif', 'png', 'jpeg', 'zip', 'rar'); //设置上传文件类型
    ...
    public function _doThum(&$info)
    {
        ...
        //设置需要生成缩略图,仅对图像文件有效
        //读取配置文件中的设置
        $imgtbSize  = explode(',', (C('CFG_IMGTHUMB_SIZE') ? C('CFG_IMGTHUMB_SIZE') : '300X300'));
        $imgtbArray = array();
        ...
        //读取配置文件固定宽等比缩略
        $imgtbFixWidth = explode(',', (C('CFG_IMGTHUMB_WIDTH') ? C('CFG_IMGTHUMB_WIDTH') : 600));
        $imgtbFixArray = array();
