...
class MemberController extends CommonController {
    public function index(){
    	...
    	if (!empty($keyword)) {
            //不同字段相同的查询条件
            $where['member.email|member.username'] = array('LIKE', "%{$keyword}%");
    	}
    	...
    	$list = D('MemberView')->where($where)->order('id desc')->limit($limit)->select();
    ...
    public function edit(){
    	...
    	$this->vo = M('member')->find($id);
    	$this->assign('type','编辑会员');
    	$this->display();
    }
